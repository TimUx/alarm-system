...
Der Installer (`install.sh`) richtet udev-Zugriff ein und bindet `/dev/cec0` an den alarm-monitor-Container. `cec-client`/`cec-utils` sind im alarm-monitor-Image enthalten (kein Host-Binary-Mount). Optionale Host-Pakete dienen nur manuellen Tests. Bei erneutem Ausführen erkennt das Skript bestehende Installationen (Upgrade-Modus). Die eigentliche Steuerungslogik liegt in **alarm-monitor** und wird über Einstellungen → HDMI-CEC konfiguriert.
